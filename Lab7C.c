...
int main()
{
    ...
    n=findRoot(start, end, n, coefficients, &root);
...
if(fabs((func(left)*func(est)))<EPSILON)
   {
       *root=est;
        flag=1;
        rootfound=1;
...
*/
void plotFunc(double begin, double end, int nRoots, double root)
{
    double x[N];
    double y[N];
    double inc;
...
void plot(int n, double *xPtr, double *yPtr, int nRoots, double xRoots, double yRoots)
{
    ...
    {
        plcol0(RED);
       //plpoin(1,xRoots, yRoots, 'x');
       plptex(xRoots,yRoots,0,0,0.5,"x");
    }
    plend();
}
