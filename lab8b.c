...
void calculateAreaTrapazoid(SHAPE *sPtr)
{
 ...
 double sum = 0;
 double h;
 double aval = sPtr->aStart;
 for (x=0; x<sPtr->n;x=x+1)
 {
    sPtr->a[x] = aval;
    h = 2*aval/sPtr->num_steps;
    x1= h - aval;
    sum = 0;
    for (y=1;y<=sPtr->num_steps-1;y=y+1)
    {
    sum = sum + (((pow(aval,2)- pow(x1,2))*exp((-x1)/aval)));
    x1=x1+h;
    }
    sPtr->areaTrap[x] = sum*h;
    aval= aval + sPtr->inc;
